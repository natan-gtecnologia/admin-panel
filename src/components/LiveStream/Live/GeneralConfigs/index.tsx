...
export function GenerealConfigs({ liveId, chat, timeout }: GeneralConfigProps) {
  ...
  const { mutateAsync: handleUpdateTimeout, isLoading: isTimeoutUpdating } =
    useMutation<void, void, { timeout: number }>({
  ...
  const timeoutDebounce = useCallback(
    debounce((timeout: number) => handleUpdateTimeout({ timeout }), 500),
    []
  );
  ...
  return (
    <Card>
      <Card.Header>
        <h6 className="card-title mb-0 flex-grow-1">Configurações Gerais</h6>
      </Card.Header>
      <Card.Body>
        <Row>
          <Col
            ...
            <div className="form-group">
              ...
              <Input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  timeoutDebounce(Number(e.target.value))
                }
