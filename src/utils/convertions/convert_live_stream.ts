import { convert_image_strapi } from "@growth/core/util/formatting";
import { ILiveStream } from "../../@types/livestream";
import { IStrapiLiveStream } from "../../@types/strapi";
import { convert_category_strapi } from "./convert_category";
...
export function convert_livestream_strapi(
    ...
    return {
        ...
        coupons: livestream.attributes.coupons?.data?.map(convert_coupon_strapi) ?? [],
        ...
        broadcasters: livestream.attributes?.broadcasters?.data.map(
            broadcaster => ({
                id: broadcaster.id,
